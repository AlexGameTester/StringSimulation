...
class InputWindow:
    ...
    def __init__(self, root, manager):
        ...
        simulation_time_entry = tkinter.Entry(self.root, textvariable=simulation_time)
        ...
        density_entry = tkinter.Entry(self.root, textvariable=density)
        ...
        tension_entry = tkinter.Entry(self.root, textvariable=tension)
        canvas1.create_window(200, 260, window=tension_entry)
        canvas1.create_window(200, 220, window=label3)


        self.root.start_button = tkinter.Button(bottomframe, text="Start", command=self._get_start_parameters(),   width=17, 
                                        height=3, font=18)
    ...
    def _get_start_parameters(self) -> StartParameters:
        ...
        """
        print(self.root.simulation_time_entry.get())
        
        
        
        if len(self.root.simulation_time_entry.get()) == 0:
            simulation_time_double = 10
        elif tkinter.getdouble(self.root.simulation_time_entry.get()) < 0:
            simulation_time_double = 11
        else :
            a = tkinter.getdouble(self.root.simulation_time_entry.get())
